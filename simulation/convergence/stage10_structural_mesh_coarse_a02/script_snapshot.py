...
def structural_deck(nx: int, nz: int, ramp_dt: float, hold_dt: float) -> str:
    ...
    modulus, poisson, prony = prony_lines()
    lines = [
        ...
        f"MP,EX,1,{modulus:.16g}", f"MP,PRXY,1,{poisson:.16g}", *prony,
    ]
    ...
    for iz in range(nz + 1):
        ...
        lines.append(f"D,{node(nx, iz)},UY,0")
    lines.extend([
...
def contact_deck(nx: int, ny: int, algorithm: str, fkn: float, friction: float) -> str:
    ...
    lines = [
        ...
        f"KEYOPT,3,2,{key}", "KEYOPT,3,4,0", "KEYOPT,3,10,2",
        "R,1,1", f"R,3,{fkn:.16g},0.1,0,2", "MP,EX,1,2000", "MP,PRXY,1,0.3",
...
def execute(case_id: str, attempt: int, case: dict, deck: str, parser) -> dict:
    ...
    case.update({
        ...
        "structural_revision_sha256": (
            digest(ROOT / "simulation/cases/stage10_convergence_design_revision_02.json")
...
def cases(design: dict) -> list[tuple[str, dict, str, object]]:
    ...
    for level, factor in level_factor.items():
        ...
        case = {"family": "structural_mesh", "design_revision": 2, "level": level, "nx": nx, "nz": nz, "ramp_dt_s": 0.01, "hold_dt_s": 0.1}
    ...
    for level, values in design["time_step_strategies"].items():
        if level != "fine":
            case = {"family": "structural_time", "design_revision": 2, "level": level, "nx": 60, "nz": 8, "ramp_dt_s": values["structural_ramp_dt_s"], "hold_dt_s": values["structural_hold_dt_s"]}
