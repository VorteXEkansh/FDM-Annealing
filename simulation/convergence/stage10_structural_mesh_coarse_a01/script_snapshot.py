...
def cases(design: dict) -> list[tuple[str, dict, str, object]]:
    ...
    for level, factor in level_factor.items():
        ...
        case = {"family": "structural_mesh", "level": level, "nx": nx, "nz": nz, "ramp_dt_s": 0.01, "hold_dt_s": 0.1}
    ...
    for level, values in design["time_step_strategies"].items():
        if level != "fine":
            case = {"family": "structural_time", "level": level, "nx": 60, "nz": 8, "ramp_dt_s": values["structural_ramp_dt_s"], "hold_dt_s": values["structural_hold_dt_s"]}
