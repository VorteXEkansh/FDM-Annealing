...
def main():
    ...
    record={'stage':1,'repository':'https://github.com/VorteXEkansh/FDM-Annealing','branch':branch,'content_commit':head,'remote_commit':remote,'github_api_commit':api,'pdf_git_blob':remote_blob,'pdf_sha256':hashlib.sha256((ROOT/pdf).read_bytes()).hexdigest(),'status':'Push, remote branch, GitHub commit and PDF blob verified','date':'2026-09-12'}
    if args.record:
        (ROOT/'docs/stage_01_delivery.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
