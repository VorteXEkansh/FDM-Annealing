...
def main() -> None:
    ...
    if args.execute:
        ...
        scratch_dir = Path(tempfile.gettempdir()) / "FDM-Annealing" / args.run_id
        if scratch_dir.exists():
            raise FileExistsError(f"immutable solver scratch directory already exists: {scratch_dir}")
        scratch_dir.mkdir(parents=True)
        scratch_input = scratch_dir / "input.dat"
        shutil.copyfile(input_path, scratch_input)
        shutil.copyfile(CONFIG, scratch_dir / "case.json")
        command = [str(MAPDL), "-b", "-i", str(scratch_input), "-o", str(scratch_dir / "mapdl.out"), "-j", "stage08_thermal", "-np", "1"]
        with (scratch_dir / "runner_stdout.log").open("wb") as stdout, (scratch_dir / "runner_stderr.log").open("wb") as stderr:
            completed = subprocess.run(command, cwd=scratch_dir, stdout=stdout, stderr=stderr, check=False)
        returncode = completed.returncode
        for artifact in sorted(scratch_dir.iterdir()):
            if artifact.is_file() and artifact.name not in {"input.dat", "case.json"}:
                shutil.copyfile(artifact, run_dir / artifact.name)
        if returncode != 0:
