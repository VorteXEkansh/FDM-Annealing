...
def spec(kind):
    ...
    if kind=="visco":
        ...
        d.update(E=m['instantaneous_moduli_MPa']['E'],nu=m['poisson_ratio'],prony=m['prony'],temperature_C=65.,strain=.001,ramp_s=1.,dt=.0001,hold_dt=.1,purpose="isothermal source-spectrum implementation test at 65 C; not physical validation")
    return d
...
def deck(c):
    ...
    if kind=='visco':
        lines += ['TREF,65','BF,ALL,TEMP,65','OUTRES,ALL,LAST']
        stages=[(1.,.01,c['dt']),(11.,.01,c['hold_dt']),(31.,.01,c['hold_dt']),(32.,0.,c['dt']),(62.,0.,c['hold_dt'])]
...
def compare(c,raw):
    ...
    for vals in raw:
        ...
        for i,k in enumerate(names,1):
            ...
            actual=(int(vals[i]>=2) if k=='contact_closed' else -vals[i] if k=='gap_mm' else vals[i]); expected=ref[k]; err=abs(actual-expected)
